...
class View extends Backbone.View {
    constructor(el) {
        ...
        this.bound = {
            ...
            // click: event => this.click(event)            // Development bind for testing purposes
        };
        ...
        // this.canvas.addEventListener('auxclick', event => this.addNode(event));          // Development bind for testing node adding
        // this.canvas.addEventListener('click', this.bound.click);                         // Development bind for testing purposes
        // this.canvas.addEventListener('contextmenu', event => this.contextmenu(event));   // Development Bubblegum Contextmenu, requires FF or --enable-blink-features=ContextMenu paramn in Chrome -> REDO with pure HTML5 + ES6 menu
    ...
    init() {
        ...
        Backbone.sync = function (method, model, options) {
            options.headers = { 'Authorization': 'Bearer ' + sessionStorage.token };    // Set the 'Authorization' header and get the access token from the session storage
            backboneSync(method, model, options);                                       // Call the stored original Backbone.sync method with extra headers argument added
        };
    ...
    addNode(event) {
        ...
        this.network.add(node);                                                             // Add node to network
        node.save();                                                                        // Save / POST new node to REST / database
        this.modallayer.appendChild((new Modal('editNode', node.get('id'))).dialog);        // Add and open dialog for inputting element data
    }

    keydown(event) {
        if(event.keyCode === 17) { this.ctrlKey = true; }
    }

    keyup(event) {
        if(event.keyCode === 17) { this.ctrlKey = false; }
    }

    zoom(event) {
        event.preventDefault();
        const scale2 = ((100 - Math.sign(event.deltaY) * 4) / 100);
        this.scale = this.scale * scale2;
        this.context.scale(scale2, scale2);
        if(this.requestID === undefined) { this.requestID = requestAnimationFrame(() => { this.renderLoop(performance.now(), 0); }); }
    }
    ...
    doubleclick(event) {
        ...
        if(hit !== null) { this.modallayer.appendChild((new Modal(hit.get('type'), hit.get('id')).dialog)); }
    }
